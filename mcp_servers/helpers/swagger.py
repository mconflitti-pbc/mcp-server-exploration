...
def expand_refs(document, obj):
    """
    Expands `ref`s in the given object. Returns an object semantically
    equivalent to the original but with references expanded.

    :param document: the master swagger document containing the
    responses and definitions.
    :param obj: is either a normal swagger object, a ref object,
    or a swagger object with a schema.
    """
...
def expand_all_references(document):
    ...
    error_responses = [
        ...
        "NotFound", "Conflict", "APIError", "InternalServerError"
    ]
...
def transform_swagger_to_operation_dict(swagger_dict):
    """
    Transforms the structure of a Swagger dictionary to create a dictionary where each entry key is the operation ID
    and the value is the definition for that operation, including the HTTP verb and the route.
